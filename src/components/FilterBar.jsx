  ...
  return (
    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 animate-fade-in">
      ...
      <select
        ...
        className="w-full sm:w-auto px-4 py-2.5 rounded-xl glass-input text-sm text-slate-700 focus:outline-none transition-all cursor-pointer"
      >
        {statuses.map((s) => (
      ...
      {(search || statusFilter !== 'ALL') && (
        <button
          onClick={onClear}
          className="w-full sm:w-auto px-4 py-2.5 rounded-lg border border-slate-200 bg-white text-sm text-slate-500 hover:text-rose-600 hover:border-rose-200 hover:bg-rose-50 transition-all cursor-pointer"
        >
          Clear
        </button>
      )}
